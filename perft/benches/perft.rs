...
fn bench_startpos_d3(c: &mut Criterion) {
    ...
    c.bench_function("perft startpos d3", |b| b.iter(|| black_box(perft_count(&board, 3))));
}

fn bench_kiwipete_d2(c: &mut Criterion) {
    let fen = "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1";
    let board = Board::from_fen(fen).expect("failed to parse kiwipete");
    c.bench_function("perft kiwipete d2", |b| b.iter(|| black_box(perft_count(&board, 2))));
}
...
fn bench_pos3_d4(c: &mut Criterion) {
    ...
    c.bench_function("perft pos3 d4", |b| b.iter(|| black_box(perft_count(&board, 4))));
}
