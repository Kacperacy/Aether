...
/// Pure Alpha-Beta searcher - baseline implementation without optimizations
///
/// Features:
/// - Iterative deepening
...
/// - Futility pruning
/// - Aspiration windows
pub struct PureAlphaBetaSearcher<E: Evaluator> {
    evaluator: E,
    info: SearchInfo,
...
impl<E: Evaluator> PureAlphaBetaSearcher<E> {
    ...
    fn alpha_beta(
        ...
        for mv in moves.iter() {
            ...
            let score = -self.alpha_beta(board, depth - 1 + extension, ply + 1, -beta, -alpha);
...
impl<E: Evaluator + Send> Searcher for PureAlphaBetaSearcher<E> {
    fn search(
        &mut self,
        board: &mut Board,
        limits: &SearchLimits,
        ...
        for depth in 1..=max_depth {
            ...
            // Full-window search (no aspiration)
            let score = self.alpha_beta(board, depth, 0, NEG_MATE_SCORE, MATE_SCORE);
...
impl<E: Evaluator + Default> Default for PureAlphaBetaSearcher<E> {
    fn default() -> Self {
        Self::new(E::default())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_pure_search_basic() {
        let evaluator = SimpleEvaluator::new();
        let mut searcher = PureAlphaBetaSearcher::new(evaluator);
